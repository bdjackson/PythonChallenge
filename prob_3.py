...
# ----------------------------------------------------------------------------
def bodyGuards(char_group):
  # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  if len(char_group) == 0: return False
  
  sub_str = char_group[1:-1]
  body_guards = char_group[0].islower() and char_group[-1].islower()
  for ch in sub_str:
    body_guards = body_guards and ch.isupper()
  
  return body_guards

# ----------------------------------------------------------------------------
def findLowerWithBodyguards(in_string, num_bodyguards):
  # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  chars = ""
  for i, ch in enumerate(in_string):
    if ch.islower():
      left_chars  = in_string[i-num_bodyguards-1:i+1]
      right_chars = in_string[i:i+1+num_bodyguards+1]
      if bodyGuards(left_chars) and bodyGuards(right_chars):
        chars += ch
...
def main():
  # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  # print sorted(findLowerWithBodyguards(page_source, 3))
  chars = findLowerWithBodyguards(page_source, 3)
  print chars
